...
export const JobTracker: React.FC<JobTrackerProps> = ({ onClose }) => {
    ...
    // #10 — Campo de busca para filtrar vagas no kanban
    const [search, setSearch] = useState('');
    // #5 — Substitui window.confirm por estado de confirmação inline
    ...
    const addJob = (status: JobStatus) => {
        ...
        const job: JobApplication = { id: crypto.randomUUID(), company: newJob.company, role: newJob.role, status, dateAdded: new Date().toISOString() };
        saveJobs([...jobs, job]); setNewJob({ company: '', role: '' }); setIsAdding(null);
    };

    // #5 — deleteJob sem confirm() nativo: usa estado inline
    const deleteJob = (id: string) => setConfirmDelete(id);
    const confirmDeleteJob = () => { if (confirmDelete) { saveJobs(jobs.filter(j => j.id !== confirmDelete)); setConfirmDelete(null); } };
            ...
            <div ref={modalRef} className="bg-white dark:bg-slate-950 flex flex-col h-[90vh] w-[95vw] max-w-7xl mx-auto mt-8 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-scale-in" onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" tabIndex={-1}>
                ...
                <div className="h-16 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 bg-white dark:bg-slate-900">
                    ...
                    <div className="flex items-center gap-3">
                        {/* #10 — Campo de busca para filtrar vagas */}
                        <div className="relative hidden sm:block">
                            ...
                            <input
                                ...
                                className="pl-8 pr-4 py-1.5 text-xs border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-800 dark:text-slate-200 outline-none focus:ring-2 focus:ring-trampo-500/20 w-56"
                            />
                        </div>
                ...
                <div className="flex-1 overflow-x-auto overflow-y-hidden p-6 bg-slate-50 dark:bg-slate-950">
                    <div className="flex gap-4 h-full min-w-max">
                        {COLUMNS.map(col => {
                            // #10 — Filtra vagas pelo termo de busca
                            const colJobs = jobs.filter(j => j.status === col.id &&
                                (!search || j.role.toLowerCase().includes(search) || j.company.toLowerCase().includes(search)));
                            return (
                                    ...
                                    <div className="flex-1 overflow-y-auto p-2 space-y-3 custom-scrollbar">
                                        ...
                                        {colJobs.map(job => (
                                            <div key={job.id} draggable onDragStart={(e) => handleDragStart(e, job.id)} className="bg-white dark:bg-slate-900 p-3.5 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 cursor-move hover:shadow-md hover:-translate-y-1 transition-all duration-200 group relative">
                                                <div className="flex justify-between items-start mb-1">
                                                    ...
                                                    {/* #5 — Confirmação inline em vez de window.confirm */}
                                                    {confirmDelete === job.id ? (
                                                        <div className="flex items-center gap-1 animate-scale-in">
                                                            <button onClick={confirmDeleteJob} className="p-1 bg-red-500 text-white rounded hover:bg-red-600 transition-colors" title="Confirmar exclusão"><Check size={12} /></button>
                                                            <button onClick={() => setConfirmDelete(null)} className="p-1 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded hover:bg-slate-300 transition-colors" title="Cancelar"><X size={12} /></button>
                                                        </div>
                                                    ) : (
                                        ...
                                        {isAdding === col.id ? (
                                            <div className="bg-white dark:bg-slate-900 p-3 rounded-lg border-2 border-trampo-500 shadow-lg animate-scale-in">
                                                {/* #14 — Enter para submeter o formulário de vaga */}
                                                <input autoFocus className="w-full text-sm font-bold mb-2 bg-transparent outline-none border-b border-slate-200 dark:border-slate-700 dark:text-white" placeholder="Cargo (ex: Frontend)" value={newJob.role} onChange={e => setNewJob({ ...newJob, role: e.target.value })} onKeyDown={e => { if (e.key === 'Enter') addJob(col.id); if (e.key === 'Escape') setIsAdding(null); }} />
                                                <input className="w-full text-xs mb-3 bg-transparent outline-none dark:text-slate-300" placeholder="Empresa" value={newJob.company} onChange={e => setNewJob({ ...newJob, company: e.target.value })} onKeyDown={e => { if (e.key === 'Enter') addJob(col.id); if (e.key === 'Escape') setIsAdding(null); }} />
